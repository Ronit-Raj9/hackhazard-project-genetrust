...
import { Info, AlertTriangle, ChevronDown, ChevronUp, Zap, ExternalLink, Check, Wallet, LogIn, Copy as CopyIcon } from 'lucide-react';
...
const WalletConnector: React.FC<WalletConnectorProps> = ({
  ...
  useEffect(() => {
    ...
    const unsubscribe = authEvents.subscribe((event: string, data?: any) => {
      if (event === 'auth_state_changed' || event === 'refresh_requested') {
        console.log(`WalletConnector: ${event} event received`, data);
        ...
        if (data && data.isAuthenticated === false) {
          ...
          // Check for dependent props to avoid unnecessary operations
          if (forceAuthenticated === undefined) {
            // Disconnect wallet if still connected after logout
            if (isConnected) {
              disconnect();
        ...
        // Always check auth state for any auth event
        // Check auth state from localStorage/cookies more reliably
        const token = localStorage.getItem('auth_token');
        const guestId = getGuestId();
        const isGuestActive = isGuestSessionActive();
        const localAuthState = !!token || (!!guestId && isGuestActive);
        
        // If our auth state doesn't match local storage, request a refresh
        if (localAuthState !== isAuthenticated && forceAuthenticated === undefined) {
          ...
          // Emit refresh_requested event, but avoid loops by only emitting if data differs
          if (event !== 'refresh_requested') {
    ...
    if (forceAuthenticated === undefined) {
      ...
      if (localAuthState !== isAuthenticated) {
        console.log('WalletConnector: Initial auth state mismatch detected', {
          localAuthState,
          isAuthenticated,
          hasToken: !!token,
          hasGuestId: !!guestId,
        ...
        // Request refresh on mount if needed
        setTimeout(() => authEvents.emit('refresh_requested'), 100);
      }
    }
  ...
  }, [isAuthenticated, isConnected, disconnect, setWalletConnected, forceAuthenticated]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.log('Failed to save wallet connection data');
    }
  }, [isGuest, isConnected, address, chainId, setWalletConnected, isMounted, isAuthenticated]);
  ...
  const onDisconnect = () => {
    ...
    // Notify about wallet disconnection with full state information
